...
未携带或携带错误 API Key 返回 401；不存在音色返回 404 `voice_not_found`；超长文本、空文本、错误 Content-Type 返回 4xx。`/speak` 默认限流为每分钟 30 个请求，可用 `TTS_RATE_LIMIT_PER_MINUTE` 调整。内部 `/synthesize` 仅允许 `127.0.0.1` / `::1` 且仍需 API Key，不消耗公共限流额度，缓存命中可直接复用。
