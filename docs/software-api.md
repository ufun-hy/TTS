...
| 接口 | 作用 |
| --- | --- |
| `POST /api/live/start` | `{ "voice": "default", "playback_speed": 1.0, "volume": 100, "segments": [{ "id": "p0001", "text": "确认后的直播文本" }] }`，创建并异步启动 Session；速度和音量在 Mac 入 Audio Cache 前处理 |
...
Live Session 只接收 Text Studio 已确认的自然段文本，保持传入顺序，调用 TTS Gateway `/synthesize`，按 Session 的 `playback_speed` 和 `volume` 让 Audio Cache Processor 生成最终 WAV，再送入 Audio Cache `/audio/enqueue`。仅当某一段超过 Gateway 单次长度限制时，才按固定字符块生成 `p0005-01` 这类技术子段；不会重新做语义断句或泛化。Windows 客户端继续使用现有 `/audio/next` 拉取和 ACK 流程，播放层不再调速或调音量。
