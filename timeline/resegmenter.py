...
def _pack_units(texts: Sequence[str], config: ResegmentConfig) -> List[str]:
    ...
    for text in texts:
        ...
        if _duration(current) < config.min_duration and _duration(combined) <= config.max_duration:
            current = combined
...
def split_text(text: str, config: ResegmentConfig) -> List[str]:
    ...
    return _pack_units(base, config)
...
def _timestamped_units(raw: Dict[str, Any], config: ResegmentConfig) -> Optional[List[Unit]]:
    ...
    if isinstance(values, list) and values:
        ...
        for value in values:
            if not isinstance(value, dict) or not normalize_text(value.get("text")):
                continue
            try:
                result.append(Unit(normalize_text(value["text"]), float(value["start"]), float(value["end"])))
    ...
    for value in words:
        ...
        if current and (gap >= config.pause_threshold or re.search(r"[。！？!?；;]$", current[-1])):
            result.append(Unit("".join(current), current_start, previous_end))
            current = []
            current_start = None
        ...
        previous_end = end
        if re.search(r"[。！？!?；;]$", word):
            result.append(Unit("".join(current), current_start, end))
            current = []
            current_start = None
    if current:
        result.append(Unit("".join(current), current_start, previous_end))
...
def _split_timestamped(unit: Unit, config: ResegmentConfig) -> List[Unit]:
    if unit.start is None or unit.end is None or unit.end <= unit.start:
        return [Unit(text) for text in split_text(unit.text, config)]
    if unit.end - unit.start <= config.max_duration:
        return [unit]
    texts = split_text(unit.text, config)
    weights = [_duration(text) for text in texts]
    total = sum(weights) or 1
    result = []
    cursor = unit.start
    for index, (text, weight) in enumerate(zip(texts, weights)):
        end = unit.end if index == len(texts) - 1 else cursor + (unit.end - unit.start) * weight / total
        result.append(Unit(text, cursor, end))
        cursor = end
    return result
...
def resegment_segment(raw: Dict[str, Any], index: int, config: ResegmentConfig) -> List[Dict[str, Any]]:
    ...
    if timestamped:
        ...
        units = _estimate_times(split_text(text, config), start, end)
    if not units:
    ...
    for child_index, unit in enumerate(units, 1):
        ...
        child_start = start if child_index == 1 else float(result[-1]["end"])
        child_end = end if child_index == len(units) else float(unit.end)
        result.append({
            "id": child_id,
